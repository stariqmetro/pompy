...
try:
    ...
    fact_table = pd.read_csv(MANIFEST_DETAILS_PATH)
...
manifest_table = fact_table.drop(columns=['cost1','vendor1','cost2','vendor2','StartDate',
                                          "CountLegs", "CountLegs2", "is_standard", "is_standard2"])
...
fact_table1 = fact_table.loc[:, ["cost1",'vendor1','lane1','key1',"Legs", "CountLegs", "is_standard"]]
fact_table1.rename(columns={"cost1": 'cost', "vendor1": 'vendor', "lane1": 'lane',
                            "key1": 'key', "Legs": 'leg', "CountLegs": 'count_leg'},
                   inplace=True)
fact_table1.dropna(subset=['key'])
fact_table2 = fact_table.loc[:, ['cost2','vendor2','lane2','key2',"Legs2", "CountLegs2", "is_standard2"]]
fact_table2.rename(columns={"cost2": 'cost', "vendor2": 'vendor', "lane2": 'lane',
                            "key2": 'key', "Legs2": 'leg', "CountLegs2": 'count_leg',
                            "is_standard2": "is_standard"},
                   inplace=True)
...
print("\nAllocating cost to legs...")
fact_table["Leg Cost"] = fact_table.apply(al.leg_cost, axis=1)
...
print("\nExploding columns...")
explode_columns = ['leg', 'count_leg', 'is_standard', 'Zips', 'Leg Distance', 'Leg Cost', 'Leg Manifest']
...
df_poms = pd.concat((pd.read_csv(f, encoding='ISO-8859-1', engine='c', dtype={'OrderNo': str}, usecols=['Manifest No',
                    'OrderNo', 'GoodsType', 'Weight', 'Cu_Ft_', 'Location', 'Warehouse']
...
kpi_files = iglob(os.path.join(KPI_PATH, "*.csv"))
df_kpi = pd.concat((pd.read_csv(f, encoding='ISO-8859-1', engine='c', dtype={'Order #': str}, usecols=['Order #',
                                'Actual Delivery Date', 'Client Name', 'Amount']
